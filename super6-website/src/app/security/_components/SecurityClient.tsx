...
type Props = {
  sections: SecuritySectionType[];
};

const HEADER_OFFSET = 96;

function buildSlugSet(sections: SecuritySectionType[]): Set<string> {
  const s = new Set<string>();
  sections.forEach((sec) => sec.items.forEach((i) => s.add(i.slug)));
  return s;
}

function buildSectionForSlug(
  sections: SecuritySectionType[]
...
export default function SecurityClient({ sections }: Props) {
          ...
          {!hasResults ? (
            <div className="faq-empty" role="status">
              ...
              <p className="faq-empty-body">
                Try a different keyword — bag check, gate, weapons, ejection,
                refunds — or{" "}

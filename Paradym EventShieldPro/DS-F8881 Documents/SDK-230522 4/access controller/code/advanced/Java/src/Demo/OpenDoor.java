...
//关闭等待
    ...
    public void open() {
        ...
        openDoor_parameter.Door.SetDoor(2, 0);//设定2号门不执行操作
    ...
    public void readPasswordDataBaseDetail(){
        CommandDetail commandDetail = new CommandDetail();
        commandDetail.Timeout = 2000;
        TCPClientDetail tcpClientDetail = new TCPClientDetail("192.168.1.251", 8000);//IP地址，端口(默认8000)
        commandDetail.Connector = tcpClientDetail;
        commandDetail.Identity = new Door8800Identity("MC-5812T19084241", "FFFFFFFF", E_ControllerType.Door8800);//设置SN(16位字符)，密码(8位十六进制字符)，设备类型
        CommandParameter par = new CommandParameter(commandDetail);
    ...
    public void readPushButtonSetting() {
        CommandDetail commandDetail = new CommandDetail();
        // commandDetail.Event = this;
        TCPClientDetail tcpClientDetail = new TCPClientDetail("192.168.1.65", 8000);//IP地址，端口(默认8000)
        commandDetail.Connector = tcpClientDetail;
    ...
    public void CommandCompleteEvent(INCommand cmd, INCommandResult result) {
        System.out.println("Demo.OpenDoor.CommandCompleteEvent() 操作成功");
        ...
        if (result instanceof ReadPushButtonSetting_Result) {
            ReadPushButtonSetting_Result incr = (ReadPushButtonSetting_Result) result;
            System.out.println("是否启用：" + incr.Use);
            System.out.println("门号：" + incr.DoorNum);
            System.out.println("是否启用出门按钮常开功能：" + incr.NormallyOpen);
            System.out.println("开门时段：" + incr.TimeGroup.GetIndex());
        }
        // if(result instanceof ReadPasswordDataBase_Result){
        //     ReadPasswordDataBase_Result rpdbr=(ReadPasswordDataBase_Result) result;
        //      deletePassword(rpdbr.PasswordDetails);
        //  }
    }
